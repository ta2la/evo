...
{
     ObPointRel* result = new ObPointRel(gid_, index_, xy_, offset_);
     result->xySourceBox_ = xySourceBox_;
     return result;
}
...
void ObPointRel::recalculate()
{
    ...
    for (int i = 0; i < objects.count(); i++) {
        ...
        ObjectDisplable* obi = objects.get(i);
        xy_ = obi->points().get(index_);
        xySourceBox_ = obi->boundDisplable();
        break;
    }
