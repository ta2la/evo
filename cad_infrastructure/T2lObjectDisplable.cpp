...
ObjectDisplable::ObjectDisplable() :
    points_(this),
    parent_(ActiveFile::activeGet()->file())
{
    assert(parent_);
}
...
{
    if (parent_ == nullptr) return;
    bool result = parent_->objects_.objects_.removeOne(this);
    assert(result);
}
...
void ObjectDisplable::modifiedSet_()
{
    ...
    if (parent_ != nullptr) parent_->dirty_ = true;
    ...
    for ( int i = 0; i < file->objects().count(); i++ ) {
        ...
        for ( int pi = 0; pi < objecti->points().count(); pi++ ) {
            ...
            cout << "MODIFIED set - gid: " << objecti->gid() << endl;
        }
    }
}
...
void ObjectDisplable::displayChange_(EntityList& list)
{
    ...
    if ( isSelected() ) {
    ...
    else if ( CAD_SETTINGS.nonActiveDisplayGray() ) { //non-active files are gray
        ...
        if ( ActiveFile::activeGet() == nullptr ) {
            ...
            if ( parent() != ActiveFile::activeGet()->file() ) {
                changeActive = change;
            }
        }
    }
    ...
    if ( isTentative() ) {
        static StyleChange* change = new StyleChange(Color::GREEN, 0.3);
        changeActive = change;
    }
...
//=========================================================================
double ObjectDisplable::area()
{
    if (points_.count() < 3) return 0;
    Area2 area;
    for (int i = 0; i < points_.count(); i++) {
