...
{
    if (CAD_SETTINGS.grid() > 0) {
        choice_ = CHOICE_GRID;
    }
    else if ( CAD_SETTINGS.grid() == 0 ) {
        choice_ = CHOICE_POINT_VIRTUAL;
    }
    else if ( CAD_SETTINGS.grid() == -1 ) {
    ...
    TentativeImplementation::enterTentative_(Point3F(PT.x(), PT.y(), 0), view);
}

//=============================================================================
void TentativeImplementationCad::consumeEx()
{
    for (int i = 0; i < WidgetFile::col().count(); i++) {
        if ( WidgetFile::col().get(i)->entityPack()->scene() != scene_ ) continue;
        scene_->dirtySet();
        break;
    }
}
