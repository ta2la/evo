...
//===================================================================
Cmd_draw_refBoxDevelop::Cmd_draw_refBoxDevelop(void) :
    Cmd("draw ref_line"),
    gid0_(0)
{
}
...
//===================================================================
void Cmd_draw_refBoxDevelop::enterPoint( const Point2F& pt, Display& view )
{	UpdateLock l;
    ...
    if (points_.count() == 0) {
        ...
        if ( glue ) {
            ...
            if ( CAD_SETTINGS.grid() < -1 ) offset_ =  Vector2F( pt, CmdQueue::queue().originalPoint2());
        }
        ...
        if ( glue ) {
            ...
            if ( CAD_SETTINGS.grid() < -1 ) offset1 =  Vector2F( pt, CmdQueue::queue().originalPoint2() );
        }

        points_.add(Point2<double>(pt.x(), pt.y()).add(offset1));

        if ( gid0_<=0 && gid1<=0 ) {
            new CadObject_refBoxDevelop(points_.get(0), points_.get(1), file);
        }
        else {
            CadObject_refBoxDevelop* line = new CadObject_refBoxDevelop(file);
...
//===================================================================
void Cmd_draw_refBoxDevelop::enterReset( Display& view )
{
    points_.clean();
...
//===================================================================
void Cmd_draw_refBoxDevelop::enterMove( const Point2F& pt, Display& view )
{   
...
//===================================================================
QString Cmd_draw_refBoxDevelop::dialogTml() const
{
    QString result;
...
//===================================================================
QString Cmd_draw_refBoxDevelop::hint(void) const
{
    if (points_.count() == 0) {
