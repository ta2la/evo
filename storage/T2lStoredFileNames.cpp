...
void StoredFileNames::loadForImage(Afile& afile, const QString& imagePath)
{
    ...
    if (fi.exists() == false) {
        ...
        record->attrAdd(AfileAttr("type",     "file_info"));
        record->attrAdd(AfileAttr("category", "DEFAULT"));
...
QString StoredFileNames::getVariable(const char* name)
{
    ...
    Afile afile;
    afile.load(settingsFileName);
    if (afile.recordCount() == 0) return "";
    AfileAttr* ann_generator = afile.recordGet(0)->attrGet(name);
