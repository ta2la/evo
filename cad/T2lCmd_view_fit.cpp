...
{
    Point2F originNew (
        ( view.scale() > 0 ) ? box.x().beg() : box.x().end(),
        ( view.scale() > 0 ) ? box.y().beg() : box.y().end() );

    view.originSet(originNew);

    double scaleX = view.getRefCanvas().scale()*view.size().w()/box.x().getLength();
    double scaleY = view.getRefCanvas().scale()*view.size().h()/box.y().getLength();
